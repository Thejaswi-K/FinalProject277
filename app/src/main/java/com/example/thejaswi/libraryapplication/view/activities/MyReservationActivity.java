...
import com.example.thejaswi.libraryapplication.view.fragment.MyReservationAdapter;
...
public class MyReservationActivity extends AppCompatActivity {
    ...
    Call<List<Catalog>> reservedCatalogsList=mAPIService.getReservedCatalogs();
    TextView topLabel;
    MyReservationAdapter mra;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_my_order);
        topLabel = (TextView) findViewById(R.id.top_label);
        topLabel.setText("Reservations");
        recyclerView=(RecyclerView) findViewById(R.id.orderlist);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        mra=new MyReservationAdapter(this,catalogList);
        ...
        reservedCatalogsList.enqueue(new Callback<List<Catalog>>() {
            ...
            public void onResponse(Call<List<Catalog>> call, Response<List<Catalog>> response) {
                ...
                Log.e("Reservation Activity","Catalog Reserved received as :"+catalogList);
                if(response.code()==200) {
                    if (catalogList.size() == 0) {
                        Toast.makeText(getApplicationContext(), "No record found for books reserved", Toast.LENGTH_LONG).show();
                    }
                    else {
                        recyclerView.setAdapter(mra);
                        mra.setCatalogList(catalogList);
